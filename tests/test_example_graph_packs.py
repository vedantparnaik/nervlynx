...
def test_delivery_robot_pack_graph_runs() -> None:
  reg = PluginRegistry()
  register_builtin_plugins(reg)
  rt = PipelineRuntime()
  cfg = load_graph_config(Path("examples/robot_packs/delivery.yaml"))
  wire_graph_from_config(rt, reg, cfg)
  seed = rt.publish(
    topic=str(cfg["seed_topic"]),
    source="test",
...
def test_warehouse_robot_pack_graph_runs() -> None:
  reg = PluginRegistry()
  register_builtin_plugins(reg)
  rt = PipelineRuntime()
  cfg = load_graph_config(Path("examples/robot_packs/warehouse.yaml"))
  wire_graph_from_config(rt, reg, cfg)
  seed = rt.publish(
    topic=str(cfg["seed_topic"]),
    source="test",
